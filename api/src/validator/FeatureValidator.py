...
class FeatureValidator:
    ...
    def notExistsByKey(self, key):
        ...
        if self.service.feature.existsByKey(key):
            raise GlobalException(message=f'Feature already exists. Key: {c.SINGLE_QUOTE}{key}{c.SINGLE_QUOTE}', status=HttpStatus.BAD_REQUEST)

    @ValidatorMethod(requestClass=str)
    def existsByKey(self, key):
        self.validator.common.strNotNull(key, 'key')
        if not self.service.feature.existsByKey(key):
            raise GlobalException(message=f'Feature does not exists. Key: {c.SINGLE_QUOTE}{key}{c.SINGLE_QUOTE}', status=HttpStatus.NOT_FOUND)
        ...
        for key in featureKeyList :
            for feature in featureList :
                if not feature.key in featureKeyList :
                    raise GlobalException(message=f'Feature does not exists. Key: {c.SINGLE_QUOTE}{key}{c.SINGLE_QUOTE}', status=HttpStatus.BAD_REQUEST)
